...
namespace alvin0319\BedrockQuest\session;
...
use alvin0319\BedrockQuest\quest\EntityQuest;
...
final class QuestSession{
	...
	public function onCommandInvoke(PlayerCommandPreprocessEvent $event) : void{
		...
		if($commandString[0] === "/" || str_starts_with($commandString, "./")){ // blame shoghi
			...
			if($command instanceof Command){
				foreach($this->quests as $name => $quest){
					if(($quest instanceof CommandInvokeQuest) && ($quest->getCommandObj() !== null) && $quest->getCommandObj()->getName() === $command->getName() && $args === $quest->getArgs()){
						$this->completeQuest($quest);
					}
				}
			}
